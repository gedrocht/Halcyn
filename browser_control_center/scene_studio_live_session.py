"""Server-side live session management for Scene Studio."""
...
class SceneStudioLiveSession:
    ...
    def __init__(
        ...
        self._snapshot = SceneStudioLiveSnapshot()
    ...
    def start(self, payload: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        self._log_callback("INFO", "scene-studio", "Started the live Scene Studio session.")
    ...
    def stop(self) -> dict[str, Any]:
        ...
        if stopped:
            self._log_callback("INFO", "scene-studio", "Stopped the live Scene Studio session.")
        else:
            self._log_callback(
                "WARNING",
                "scene-studio",
                (
                    "Stop requested for the live Scene Studio session, "
    ...
    def _run_loop(self) -> None:
        ...
        try:
            ...
            self._log_callback(
                "ERROR",
                "scene-studio",
                f"Live Scene Studio session crashed: {error_message}",
            )
        finally:
            with self._lock:
    ...
    def _record_frame(
        ...
        if self._last_logged_failure != failure_key:
            ...
            self._log_callback(
                ...
                "scene-studio",
                (
                    "Live Scene Studio frame submission failed with "
