"""Tests for the Halcyn browser-based Control Center."""
...
from browser_control_center.control_center_state import (
    ControlCenterState,
    JobRecord,
    ManagedProcess,
)
...
class _FakeProcess:
    """Minimal subprocess stand-in for Control Center tests."""
...
class ControlCenterServerTests(unittest.TestCase):
    """Exercise the HTTP surface of the Control Center without needing the C++ toolchain."""
    ...
    def test_job_routes_accept_requests(self) -> None:
        """Job routes should forward accepted responses from the Control Center state."""

        # These routes are thin adapters over ControlCenterState methods, so mocking
    ...
    def test_unknown_post_route_returns_not_found(self) -> None:
        """Unknown Control Center POST routes should return a 404 JSON response."""
...
class ControlCenterStateTests(unittest.TestCase):
    """Exercise the testable Control Center runtime logic directly."""

    def setUp(self) -> None:
        self.project_root = Path(__file__).resolve().parents[2]
        self.state = ControlCenterState(self.project_root)
    ...
    def _wait_for(self, predicate: Callable[[], bool], timeout_seconds: float = 2.0) -> None:
        ...
        while time.time() < deadline:
            if predicate():
                return
            time.sleep(0.02)
        self.fail("Timed out waiting for background Control Center work to finish.")
    ...
    def test_start_job_captures_output_and_success_status(self) -> None:
        ...
        with mock.patch(
            "browser_control_center.control_center_state.subprocess.Popen",
    ...
    def test_start_job_handles_process_start_failure(self) -> None:
        """Job startup failures should be captured instead of crashing the Control Center."""
    ...
    def test_start_app_uses_api_host_argument_name(self) -> None:
        """The Control Center should use the current ApiHost parameter name."""
    ...
    def test_stop_app_requests_taskkill_for_running_process(self) -> None:
        ...
        with mock.patch(
            "browser_control_center.control_center_state.subprocess.run"
        ) as patched_run:
    ...
    def test_available_tools_prefers_vswhere_when_available(self) -> None:
        ...
        with mock.patch(
            "browser_control_center.control_center_state.shutil.which", return_value="python"
        ):
            with mock.patch(
                "browser_control_center.control_center_state.subprocess.run", side_effect=fake_run
            ):
    ...
    def test_available_tools_reports_visual_studio_compiler_when_cl_is_not_on_path(self) -> None:
        ...
        with mock.patch(
            "browser_control_center.control_center_state.shutil.which",
    ...
    def test_summary_includes_expected_sections(self) -> None:
        ...
        self.assertEqual(payload["status"], "ok")
        self.assertIn("docs", payload)
        self.assertIn("sceneStudio", payload["docs"])
