...
int Application::Run() {
  ...
  http_api::ApiServer apiServer(config_.api, sceneStore, codec, runtimeLog);
  opengl_renderer::Renderer renderer(config_.renderer, sceneStore, runtimeLog);
...
scene_description::SceneDocument Application::LoadSceneFromFile(const std::string& filePath) const {
  ...
  scene_description::SceneJsonCodec codec;
