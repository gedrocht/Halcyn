...
std::vector<ValidationError> ValidateSceneDocument(const SceneDocument& document)
{
  ...
  if (document.kind == SceneKind::TwoDimensional)
  {
    ...
    {
      errors.push_back(
        {"$.vertices", "The number of 2D vertices does not line up with the selected primitive type."});
    }
  ...
  if (!scene.indices.empty())
  {
    ...
    {
      errors.push_back(
        {"$.indices", "The number of indices does not line up with the selected primitive type."});
    }
  ...
  {
    errors.push_back(
      {"$.vertices", "The number of 3D vertices does not line up with the selected primitive type."});
  }
  ...
  if (!IsApproximatelyZero(SquaredLength(forward)) && !IsApproximatelyZero(SquaredLength(scene.camera.up)))
  {
    const Vector3Value perpendicular = Cross(forward, scene.camera.up);
    if (IsApproximatelyZero(SquaredLength(perpendicular)))
    {
      errors.push_back(
        {"$.camera.up", "camera.up must not point in the same direction as the camera view direction."});
    }
  }
...
RenderScene BuildRenderScene(const SceneDocument& document)
{
  ...
  {
    renderScene.vertices.push_back({vertex.x, vertex.y, vertex.z, vertex.r, vertex.g, vertex.b, vertex.a});
  }
