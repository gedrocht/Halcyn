...
$coverageFile = Join-Path $projectRoot ".coverage-browser-control-center-$PID"
...
try {
  ...
  & $python.Source -m coverage xml -o coverage-browser-control-center.xml
  if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Browser Control Center coverage completed successfully.'
