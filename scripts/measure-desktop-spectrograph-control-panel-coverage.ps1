...
$coverageFile = Join-Path $projectRoot ".coverage-desktop-spectrograph-control-panel-$PID"
...
try {
  ...
  & $python.Source -m coverage run --source=desktop_spectrograph_control_panel -m unittest discover -s desktop_spectrograph_control_panel/tests -p "test_*.py"
  if ($LASTEXITCODE -ne 0) {
  ...
  & $python.Source -m coverage xml -o coverage-desktop-spectrograph-control-panel.xml
  if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Desktop spectrograph control panel coverage completed successfully.'
