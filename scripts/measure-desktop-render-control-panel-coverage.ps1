...
$coverageFile = Join-Path $projectRoot ".coverage-desktop-render-control-panel-$PID"
...
try {
  ...
  & $python.Source -m coverage run --source=desktop_render_control_panel -m unittest discover -s desktop_render_control_panel/tests -p "test_*.py"
  if ($LASTEXITCODE -ne 0) {
  ...
  & $python.Source -m coverage xml -o coverage-desktop-render-control-panel.xml
  if ($LASTEXITCODE -ne 0) {
...
Write-Host 'Desktop render control panel coverage completed successfully.'
